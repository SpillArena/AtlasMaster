...
import { CategoryPicker, ModePicker, PacePicker } from './components/menu'
...
function App() {
  ...
  // ett steg tilbake: ledertavle > tempo > modus > kategori
  const goBack = () => {
    if (showLeaderboard) {
      setShowLeaderboard(false)
    } else if (pace) {
      ...
      >
        ...
        <main className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          {showLeaderboard ? (
            ...
            <ModePicker category={category} onPick={setMode} />
          ) : !pace ? (
            <PacePicker
              category={category}
              mode={mode}
